...
colnames(banks) = c("age", "job_category", "marital_status", "education_level", "previous_default", "account_balance", "home_owner", "current_loan", "contact_method", "day_of_month", "month", "call_duration", "campaign_number", "days_from_previous_contact", "number_of_previous_contacts", "previous_outcome", "subscribed")
...
```{r}
max(banks$campaign_number)

banks |>
  count(campaign_number) |>
  ggplot(aes(x = campaign_number, y = n)) +
  geom_col() +
  labs(title = "Number of Times Individuals were Contacted in Various Months", x = "Month", y = "Total Number of Contacts")
```
...
```{r}
banks |>
  ggplot(aes(x = days_from_previous_contact)) +
  geom_histogram() +
...
```{r}
banks |>
  ggplot(aes(x = number_of_previous_contacts)) +
  geom_histogram() +
