...
set.seed(124)
trainIndex <- createDataPartition(bank$y, p= .7, list = F) # p = proportion of data in train
...
```{r}
fitControl<-trainControl(method="repeatedcv",number=5,repeats=1, p = 0.05, classProbs = T, summaryFunction = mnLogLoss) 

set.seed(123)

fit.step = train(factor(y)~., data = training, method = "glmStepAIC", trControl =fitControl, metric = "logLoss")
coef(fit.step$finalModel)

step_OR = exp(coef(fit.step$finalModel))
step_CI = exp(confint(fit.step$finalModel, level = .95))
```

```{r}
# GLMNET 
fit.glmnet = train(factor(y)~., data = training, method = "glmnet", trControl =fitControl, metric = "logLoss")
coef(fit.glmnet$finalModel, fit.glmnet$finalModel$lambdaOpt)
```
